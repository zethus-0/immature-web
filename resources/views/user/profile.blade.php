@extends('layouts.app')
@section('content')
    <div class="lg:pb-0 pb-1/2 md:pb-1/3 ">
        @if ($errors->any())
                    ...
                    <div class="w-full md:w-3/12 md:mx-2">
                        <div class="bg-white bg-opacity-50 p-3 border-t-4 border-indigo-600">
                            <div class="image overflow-hidden">
                                <img class="h-auto w-full mx-auto" src="{{ asset('images/' . $user->image_path) }}"
                                    alt="">
                            </div>
                        ...
                        <div class="bg-white bg-opacity-50 p-3 shadow-sm rounded-sm">
                            ...
                            <div class="text-gray-700">
                                <div class="grid grid-cols-1 text-sm">
                                    @admin
                                    <div class="grid md:grid-cols-3 xs:grid-cols-4 pb-3">
                                        ...
                                        <form method="POST" action="/user/profile/{{ $user->id }}"
                                            ...
                                            @method('PUT')
                                            <input
                                                class="form-check-input appearance-none border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 bg-no-repeat bg-center bg-contain mr-2 cursor-pointer"
                                                type="checkbox" name="isadmin" value="0">
                                            <label class="form-check-label inline-block text-gray-800 mr-4"
                                                for="flexCheckDefault">
                                                Disable
                                            </label>
                                            <input
                                                class="form-check-input appearance-none border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 bg-no-repeat bg-center bg-contain mr-2 cursor-pointer"
                                                type="checkbox" name="isadmin" value="1">
                                            <label class="form-check-label inline-block text-gray-800 mr-4"
                                                for="flexCheckDefault">
                                                Enable
                                            </label>
                                    ...
                                    <div class="grid md:grid-cols-3 xs:grid-cols-4 pb-3">
                                        <div class="px-4 py-2 font-semibold">Editing Privileges</div>
                                        <div class="px-4 py-2">{{ $user->is_writer }}</div>
                                        <form method="POST" action="/user/profile/{{ $user->id }}"
                                            enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                            <input
                                                class="form-check-input appearance-none border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 bg-no-repeat bg-center bg-contain mr-2 cursor-pointer"
                                                type="checkbox" name="iswriter" value="0">
                                            <label class="form-check-label inline-block text-gray-800 mr-4"
                                                for="flexCheckDefault">
                                                Disable
                                            </label>
                                            <input
                                                class="form-check-input appearance-none border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 bg-no-repeat bg-center bg-contain mr-2 cursor-pointer"
                                                type="checkbox" name="iswriter" value="1">
                                            <label class="form-check-label inline-block text-gray-800 mr-4"
                                                for="flexCheckDefault">
                                                Enable
                                            </label>
                                    ...
                                    <form method="POST" action="/user/profile/{{ $user->id }}"
                                        ...
                                        <div class="grid md:grid-cols-3 sm:grid-cols-2 pb-3">
                                            ...
                                            <label
                                                class="p-2 mb-1 sm:w-full md:w-10/12 text-indigo-600 text-center border border-solid border-indigo-600 rounded hover:bg-indigo-600 hover:text-white transition-colors duration-300 @if (!$user->hasVerifiedEmail())
                                    cursor-not-allowed"
                                            disabled @else @endif ">
